    ...
    switch (_obj.content) {
        case message.buttons[0]:
            axios.all([
                ...
                result = message.baseType(`업비트 BTC/KRW [${upbitKRWRes.data[0].tradePrice}]
                비트파이넥스 BTC/USD [${bitfinexUSDRes.data[6]}]`);
                res.set({
                    'content-type': 'application/json'
        ...
        case message.buttons[1]:
            axios.all([
                ...
                result = message.baseType(`바이낸스 TRX/BTC [${binanceBTCRes.data.price}]
                바이낸스 TRX/ETH [${binanceETHRes.data.price}]
                코인네스트 TRX/KRW [${coinnestKRWRes.data.last}]`);
                res.set({
                    'content-type': 'application/json'
                }).send(JSON.stringify(result));
            }));
